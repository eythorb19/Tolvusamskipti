...
//
// Command line: ./server_mod 5000 
//
// Author: Jacky Mallett (jacky@ru.is)
// Modified by: Eyþór Óli Borgþórsson (eythorb19@ru.is)
//
#include <stdio.h>
#include <errno.h>
#include <stdlib.h>
                  ...
                  char *buffer) 
{
  ...
  std::stringstream response;          // Response
  char out[4096];                      // Output
  char * line;                         // Line from stream
  std::stringstream stream(buffer);    // Split command from client into tokens for parsing
  ...
  {
      FILE *command = popen(&buffer[4], "r");   // Open pipe stream, get stream from command line
      memset(out, 0, sizeof(out));              // Fills a byte string with a byte value

      // Get line from stream
     // line = fgets(out, sizeof(out), command);

      // While stream / input is not empty
      ...
      // Command invalid
      if (response.str() == "") {
          pclose(command);          // Close stream
          send(clientSocket, "Command invalid, no output \n.", sizeof("Command invalid, no output \n."), 0);
      }
      // Command valid
      else {
          pclose(command);      // Close stream
      ...
      // Input without SYS
          ...
          send(clientSocket, "Unknown command from client.\n", sizeof("Unknown command from client.\n"), 0);
    }
}
